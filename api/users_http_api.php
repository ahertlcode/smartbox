...
if (isset($data)) {
    ...
    if ($data->method == 'signin') {
        if (isset($data->data->username) && isset($data->data->password)) {
            $sout = $$obj->signin($data->data->username, $data->data->password);
            if (empty($sout)) {
                $sout = array('status' => 'error', 'msg' => 'User account not found!');
            } else {
                $sout['msg'] = 'Login, Successful!';
                //array_push($sout, array('msg' => 'Login Successful'));
            }
        } else {
            $sout = array('status' => 'warning', 'msg' => 'Please enter your email and password.');
        }
    }
